...
class CFVExp:
    ...
    def initialize_datagen(self):
        ...
        if self.cfg.selfplay.cpu_gen_threads:
            ...
            logging.info(
                "will generate on CPU with %d threads", num_threads
            )
            ...
            if (
                self.is_master
                and self.cfg.selfplay.num_master_threads is not None
            ):
                num_threads = self.cfg.selfplay.num_master_threads
            else:
                num_threads = self.cfg.selfplay.threads_per_gpu * \
                    len(act_devices)

        # don't need more devices than threads.
    ...
    def run_trainer(self):
        ...
        context: rela.Context = datagen["context"]
        ...
        for epoch in range(self.cfg.max_epochs):
            ...
            for iter_id in train_iter:
                ...
                params = (
                    self.get_policy_params() if use_policy_net
