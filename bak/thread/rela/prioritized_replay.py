...
class ConcurrentQueue:
    def __init__(
        ...
        self._tail: int = 0
        self._size: int = 0
        self._allow_write: bool = True
        self._safe_tail: int = 0
        self._safe_size: int = 0
        self._sum: float = 0.
        self._evicted: List[bool] = [False] * capacity
        self._elements: List[ValueTransition] = [None] * capacity
    ...
    def block_append(
        ...
        with self._mutex:
            self._cv_size.wait_for(
                predicate=lambda:
                    self._size + block_size <= self._capacity
                    and self._allow_write
            )
            start = self._tail
            end = (self._tail + block_size) % self._capacity
            self._tail = end
            self._size += block_size
            self._check_size(head=self._head, tail=self._tail, size=self._size)
        sum_ = 0
        assert weights.size(0) == block_size
        for i in range(block_size):
            j = (start + i) % self._capacity
            self._elements[j] = block[i]
            self._weights[j] = weights[i]
            sum_ += weights[i]
        with self._mutex:
            self._cv_tail.wait_for(
                predicate=lambda: self._safe_tail == start)
            self._safe_tail = end
            self._safe_size += block_size
            self._sum += sum_
            self._check_size(
                head=self._head,
                tail=self._safe_tail,
    ...
    def get_element_and_mark(self, idx: int) -> ValueTransition:
        ...
        """
        id_ = (self._head + idx) % self._capacity
    ...
    def _check_size(self, head: int, tail: int, size) -> None:
        if size == 0:
            ...
            assert tail + self._capacity - head == size, \
                "error: tail - head: %d vs size: %d" % (
                    tail + self._capacity - head, size
                )
