...
class RlRunner:
    ...
    def step(self) -> None:
        ...
        while not self._game.is_terminal(self._state):
            # logging.debug(
            #     msg="[proc_loop, data_gen] process id: %d, name: %s, state: %s"
            #     % (proc.pid, proc.name, self._game.state_to_string(self._state))
            # )
...
def _compute_strategy_recursive(
    ...
    for action, is_feasible in enumerate(feasible_actions):
        if is_feasible:
            if child_node_id >= node.children_end:
                print()
            ...
            _normalize_beliefs_inplace(p_beliefs=new_beliefs[state.player_id])
...
def _compute_strategy_recursive_to_leaf(
    ...
    # - add children to the queue with proper believes
