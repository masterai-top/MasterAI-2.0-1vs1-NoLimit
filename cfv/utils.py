...
class TimedContext(rela.Context):
...
def compute_exploitability(model_path, cfg_cfr_eval, cfr_binary="build/cfr"):
    ...
    if cfg_cfr_eval.args.num_processes:
        num_processes = cfg_cfr_eval.args.num_processes
    else:
        num_processes = 10 if heyhi.is_on_slurm() else 40
